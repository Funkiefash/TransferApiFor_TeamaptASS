...
public class BalanceService {
    ...
    public Balances retrieveBalances(String accountnr) {
        Balances balances = balancesRepository.findByAccountnr(accountnr)
                .orElseThrow(() -> new BalancesNotExistException("Balances id:" + accountnr + " does not exist.", ErrorCode.BALANCES_ERROR, HttpStatus.NOT_FOUND));
        return balances;
    }

    @Transactional
    public void transferBalances(TransactionRequest Transaction) throws OverDraftException, BalancesNotExistException, SystemException {
        Balances balancesFrom = balancesRepository.getBalancesForUpdate(Transaction.getfromaccountnr())
                .orElseThrow(() -> new BalancesNotExistException("Balances id:" + Transaction.getfromaccountnr() + " does not exist.", ErrorCode.BALANCES_ERROR));
